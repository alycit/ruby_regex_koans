describe "Capturing Groups" do
  ...
  # 
  # For example:
  #   matchGroups = /^(\D+)\d+(\D+)$/.exec("abc1234xyz")
  #   atchGroups.lengt.should eql(3)
  #   atchGroups[0.should eql("abc1234xyz")
  #   atchGroups[1.should eql("abc")
  #   atchGroups[2.should eql("xyz")
  # 
  ...
  it "getting the hang of it capture groups" do
    
    pattern = /^([A-Za-z][\w\-.]*(\+([\w+]+))?)@(([a-z\d]+)((\.([a-z\d]+))+))$/

    matches1 = pattern.exec( 'My.Name01+alias@mail.gmail.com' )
    matches2 = pattern.exec( 'simple@gmail.com'               )
    
    matches1[0]  .should eql('__')
    matches1[1]  .should eql('__')
    matches1[3]  .should eql('__')
    matches1[__].should eql('mail.gmail.com')
    
    matches2[2]  .should eql(__)
    matches2[__].should eql('.com')    # there are two possible answers here
  ...
  it "Real World 4: Find the ID of a DIV tag" do
    # Assumption:
    ...
    matches = fixThisPattern.exec( '<div id="anArbitraryId"> Here is my text node! </div>' )
    
    matches[__].should eql("anArbitraryId")
  ...
  it "Real World 5: Find the class of an arbitrary HTML tag" do
    # Assumption:
    ...
    matches1 = fixThisPattern.exec( '<div id="someId" class="boxed"> Some text here </div>' )
    matches2 = fixThisPattern.exec( '<span class="bold red"> Error! </span>'                )
    matches3 = fixThisPattern.exec( '<img src="image.jpg" class="framed" id="myPortrait"/>' )
    
    matches1[__].should eql("boxed")
    matches2[__].should eql("bold red")
    matches3[__].should eql("framed")
  end
  
  it "Captured groups can be referenced within the pattern itself using backslash-n" do
    ...
    thisPatternWorks = /^([a-z]+)\d+\1$/
    
    fixThisPattern = /^__$/
    
    'ab12345ab'.should match(thisPatternWorks)
    'a12345x'.should_not match(thisPatternWorks)
    
    '"Hello there"'.should match(fixThisPattern)
    '|some word here|'.should match(fixThisPattern)
    '(an expression)'.should_not match(fixThisPattern)
  end
  
  it "Real World 6: HTML tags may use single- or double-quotes" do
    # Assumption:
